...
void saveTree(bNode *root,const char filename[]){
	FILE *fp = fopen("F:\\VS Code\\VSCode Projects\\Data Structure\\Experiment\\lab5_tree\\sg.html","wb");
	if (!fp) {
		printf("打开写入文件出错！\n");
		exit(0);
	} 
	...
	FILE *fh = fopen("F:\\VS Code\\VSCode Projects\\Data Structure\\Experiment\\lab5_tree\\head.txt","rb");
	fseek(fh,0,SEEK_END); 
	long fsize = ftell(fh); 
	rewind(fh);
	...
	fh = fopen("F:\\VS Code\\VSCode Projects\\Data Structure\\Experiment\\lab5_tree\\tail.txt","rb");
	fseek(fh,0,SEEK_END); 
	fsize = ftell(fh); 
	rewind(fh);
	dataArray = (unsigned char *)malloc(sizeof(unsigned char)*fsize);
...
void writeTree(HTNode *t[],int ridx,FILE *fp){
	if (ridx>=0 && t[ridx]->w>0){
		if (t[ridx]->lchild->id >= 0){ 
			fprintf(fp,"{source: '%d-%d', target: '%d-%d', 'rela': '0', type: 'resolved'},\n",ridx,t[ridx]->w,t[ridx]->lchild->id,t[t[ridx]->lchild->id]->w); 
			writeTree(t,t[ridx]->lchild->id,fp);
		}
		if (t[ridx]->rchild->id >= 0){
			fprintf(fp,"{source: '%d-%d', target: '%d-%d', 'rela': '1', type: 'resolved'},\n",ridx,t[ridx]->w,t[ridx]->rchild->id,t[t[ridx]->rchild->id]->w);
			writeTree(t,t[ridx]->rchild->id,fp);
		}
	}
} 
#endif
