...
int createHTree(HTNode ht[], long *w){
	...
	while (i<m-numZero){ 
		...
		for(j=0;j<i;j++){
			...
			if (ht[j].parent == -1)
				if (ht[j].w < min1){
					pos2=pos1;//pos1替代pos2
					min2=min1;//min1替代min1
					pos1 = j; //j替代pos1
					min1 = ht[j].w;//ht[j].w替代min1
				} //始终保持min1<min2,找到比min1更小的ht[j].w，那么min1替代min2， ht[j].w替代min1
				else{
					if (ht[j].w < min2){ //min1 <= ht[j].w < min2 ，直接改变pos2,min2即可
						pos2=j;
						min2=ht[j].w;
					} 	
				} 
